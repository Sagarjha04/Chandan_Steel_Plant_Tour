...
db_config = {
    ...
    'database': os.environ.get('DB_NAME', 'factory_db2'),
    'connection_timeout': 10,
}

# Vercel function filesystems are ephemeral/read-only outside /tmp.
# The current report pipeline stores media as base64 in MySQL, so local disk
# is only a compatibility path for legacy media routes.
if os.environ.get("VERCEL"):
    UPLOAD_FOLDER = os.environ.get("UPLOAD_FOLDER", "/tmp/chandan_plant_tour_uploads")
else:
...
def get_db_connection():
    # Import only when a database request is actually made. This keeps the
    # Flask function boot path independent of the DB driver.
...
@app.route('/health')
def health():
    return jsonify({'status': 'ok', 'service': 'chandan-plant-tour'})
...
def scan_frame():
    # QR scanning is performed in the browser on the Vercel deployment.
    # This endpoint remains only for backward compatibility with older clients.
    return jsonify({
        'qr_text': None,
        'status': 'client_side_scanner',
        'message': 'QR scanning is handled by the browser.'
    })
