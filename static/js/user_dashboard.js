...
// 🔍 Post Image Frame to Python Server Background Decoder Router
function sendFrameToServer() {
    // Stop background frame sync execution if a valid unchecked code is already staged
    if (capturedData.qr_detail) return; 
    ...
    .then(data => {
        if (data.qr_text) {
            const incomingScannedCode = String(data.qr_text).trim();
            
            // 📊 READ WORKER ASSIGNED CHECKLIST CONFIGURATIONS FROM THE TRACKER
            const assignedCheckpointsOnPage = Array.from(document.querySelectorAll('.worker-stats-tracker-panel span[style*="font-size: 13px"]'))
                .map(node => node.textContent.replace('Cleared', '').replace('Pending', '').trim().toLowerCase());

            let targetedCheckpointCleanName = incomingScannedCode.toLowerCase();
            if (incomingScannedCode.includes("CHECKPOINT:")) {
                try { 
                    targetedCheckpointCleanName = incomingScannedCode.split("CHECKPOINT:").split("|").trim().toLowerCase(); 
                } catch(e) {}
            }

            // Verification Rule A: Check if the scanned QR code exists in the worker's roster checklist tracker
            const isAssignedToThisUser = assignedCheckpointsOnPage.some(assignedItem => 
                assignedItem.includes(targetedCheckpointCleanName) || targetedCheckpointCleanName.includes(assignedItem)
            );
            ...
            // Verification Rule B: Duplicate Checkpoint Protection Interceptor
            const clearedItemsOnPageArray = Array.from(document.querySelectorAll('.worker-stats-tracker-panel div[style*="border: 1px solid #d1fae5"]'))
                .map(node => node.textContent.replace('✓', '').replace('Cleared', '').trim().toLowerCase());

            const isAlreadyClearedToday = clearedItemsOnPageArray.some(clearedItem => 
                clearedItem.includes(targetedCheckpointCleanName) || targetedCheckpointCleanName.includes(clearedItem)
            );
